...
目标是使残差平方和最小化：$min{\beta_0, \beta_1} \sum_{i=1}^n\left(y_i-\hat{y}_i\right)^2$
...
**岭回归的公式**：岭回归的目标是最小化以下目标函数$min\beta\left\{\sum_{i=1}^n\left(y_i-\hat{y}_i\right)^2+\alpha \sum_{j=1}^p \beta_j^2\right\}$
...
**Lasso回归公式**：Lasso回归的目标是最小化以下目标函数：$min\beta\left\{\sum_{i=1}^n\left(y_i-\hat{y}_i\right)^2+\alpha \sum_{j=1}^p\left|\beta_j\right|\right\}$
